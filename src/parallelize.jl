...
# just for testing
function filter_paired_reads_very_threaded(
    reads1_path,
    reads2_path,
    query_fasta_path,
    out1_path,
    ...
    open(FASTQReader, reads1_path) do reader1
        open(FASTQReader, reads2_path) do reader2
            # to-do: are we using two threads to read two inputs?
            for chunk in zip(
                ThreadedPartition(reader1, chunk_size),
                ThreadedPartition(reader2, chunk_size)
            )
                chunk_channel = Channel{ReadPair}(chunk_size)
                Threads.@spawn _filter_chunk!(
                    chunk_channel,
                    zip(chunk...), # needed because this gives us pair of vectors not vector of pairs
                    fwd_kmerset,
                    rev_kmerset,
                    k
                )
